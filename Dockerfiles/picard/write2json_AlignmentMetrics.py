...
def write2json(alignMetricsOut, results_json):
    ...
    alignMetrics = {}
    # the last two non-empty lines are metrics and values, respectively
    ...
    d['value'] = alignMetrics['PF_HQ_ALIGNED_READS']
    d['type'] = 'reads'
    ...
    d['value'] = alignMetrics['PF_HQ_ALIGNED_Q20_BASES']
    d['type'] = 'bases'
    results['summary_metrics'].append(d)
    ...
    d = OrderedDict()
    d['label'] = 'TOTAL_READS'
    d['value'] = alignMetrics['TOTAL_READS']
    d['type'] = 'reads'
    ...
    d['value'] = alignMetrics['PF_HQ_MEDIAN_MISMATCHES']
    results['additional_metrics'].append(d)

    #mismatch rate
    d = OrderedDict()
    d['label'] = 'MISMATCH_RATE'
    d['value'] = alignMetrics['PF_MISMATCH_RATE']
    results['additional_metrics'].append(d)

    #error rate within AQ20 reads
    d = OrderedDict()
    d['label'] = 'ERROR_RATE_AQ20_READS'
    d['value'] = alignMetrics['PF_HQ_ERROR_RATE']
    results['additional_metrics'].append(d)

    #indel rate
    d = OrderedDict()
    d['label'] = 'INDEL_RATE'
    d['value'] = alignMetrics['PF_INDEL_RATE']
    results['additional_metrics'].append(d)
...
if __name__ == "__main__":
    ...
    if not options.metricfile or not options.result_json:
        logging.error(' You must provide Picard alignment summary file (--metricfile) and output json file (--result_json)')
        sys.exit(1)
    
    write2json(options.metricfile, options.result_json) 
