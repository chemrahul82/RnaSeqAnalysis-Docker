...
"""
writes output metrics from Picard CollectRnaSeqMetrics tool to json file
"""
...
def write2json(RnaSeqMetricsOut, xrRNA_basecount, results_json):
    """
    Description: Takes the output file of Picard CollectRnaSeqMetrics tool
    and generate a json file with metrics

    Takes: the Picard output file, rRNA reference mapped base count file
...
if __name__ == "__main__":
    ...
    #arguments
    parser.add_argument('-i', '--metricfile', dest = 'metricfile', help = 'The RnaSeq metrics summary output file from Picard')
    parser.add_argument('-c', '--xrRNAbasecount', dest = 'xrRNAbasecount', help = 'The file containing the counts for mapped rRNA bases')
    parser.add_argument('-j', '--result_json', dest = 'result_json', help = 'The results.json output file')
    options = parser.parse_args()
    
    if not options.metricfile or not options.result_json or not options.xrRNAbasecount:
        logging.error(' You must provide Picard alignment metric file (--metricfile), xrRNA basecount file (--xrRNAbasecount),  and output json (--result-json)')
        sys.exit(1)
    
    write2json(options.metricfile, options.xrRNAbasecount, options.result_json) 
